    ...
    pageName(name)
{
    ...
    QString style;
    style = "background-color:#1F1F1F;border-radius:" + QString::asprintf("%d", cornerRadius) + "px";
    ...
    shadow->setColor(QColor(119, 136, 153));
...
void SlidePage::AddContent(QWidget* widget) {
    ...
    pageContentContainer->addWidget(widget, false);
}

void SlidePage::AddContents(QVector<QWidget*> widgets) {
    pageContentContainer->addWidgets(widgets);
}
...
void SlidePage::RemoveContent(QWidget* widget) {
    pageContentContainer->RemoveWidget(widget);
}

void SlidePage::UpdateContents() {
    pageContentContainer->updateHeight();
}
